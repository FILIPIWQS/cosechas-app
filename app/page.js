    ...
    const iv = setInterval(() => {
      ...
      if (pending > 0) {
        ...
        if (typeof Notification !== 'undefined' && Notification.permission === 'granted') {
          try {
            new Notification('⚠️ Estoque pendente – Siembras', {
    ...
    if (pendingAll > 0) {
      ...
      const t = setInterval(() => {
        ...
        document.title = blink
          ? `⚠️ ${pendingAll} pendente${pendingAll > 1 ? 's' : ''} – Siembras`
          : 'Siembras – contagem pendente';
      }, 1200);
      return () => { clearInterval(t); document.title = 'Siembras'; };
    } else {
      document.title = '✓ Siembras – tudo contado';
    }
  // eslint-disable-next-line
