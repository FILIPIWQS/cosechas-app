...
import { buildProcedenciaPdf } from '../../../../lib/etiquetaPdf';
...
export async function POST(request) {
  ...
  try {
    ...
    if (!produto || !fornecedor || !lote || !dataValidadeOriginal || !dataManipulacao || !dataValidade || !colaborador) {
    ...
    const registro = { produto, fornecedor, lote, sif, dataValidadeOriginal, dataManipulacao, dataValidade, colaborador };
    const pdfBytes = await buildProcedenciaPdf(registro, quantidade, larguraMm, alturaMm);

    return new Response(pdfBytes, {
      headers: {
        'Content-Type': 'application/pdf',
        'Content-Disposition': 'inline; filename="etiqueta-procedencia.pdf"',
      },
    });
  } catch (e) {
